...
pluginHandler.loadPlugin = name => {
  return new Promise(function (resolve, reject) {
    ...
    try{
      let config = {};
      let configPath = pluginHandler.dir + name + '/config.json';
      console.log(statPath(configPath))
...
pluginHandler.unloadPlugin = (name, forceUnload) => {
  return new Promise(function (resolve, reject) {
    ...
    if(pluginHandler.plugins[pluginIndex].protected && !forceUnload)
      return reject(new Error('Plugin is protected and cant be unloaded'));
...
pluginHandler.loadPlugins = dir => {
  return Promise.all(getDirectories(dir).map(name => {return pluginHandler.loadPlugin(name)}));
}
